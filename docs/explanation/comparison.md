...
> **Explanation.** How `unthrown` relates to the other errors-as-values
> libraries, and when one of them is the better fit. If you're migrating, the
> how-to guides for [try/catch](../how-to/migrate-from-try-catch) and
> [neverthrow](../how-to/migrate-from-neverthrow) are more practical.

The short version: they all return failures as values; they differ on **whether
...
- Coming from neverthrow: [Migrate from neverthrow](../how-to/migrate-from-neverthrow).
- Coming from raw `try`/`catch`: [Migrate from try/catch](../how-to/migrate-from-try-catch).
- The deliberate omissions, in one place: [Design decisions](./design-decisions).
