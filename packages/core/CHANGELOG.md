...
- c3fdcf7: Harden the core against a batch of audit findings. **Minor, not patch**: two of the fixes tighten types, so code that relied on the looser signatures stops compiling (it was already wrong at runtime).
...
- 352c204: `validateAll` / `validateAllFromDict` and their async counterparts
...
- de9486c: Add `fromExecutor` — the callback-API boundary.
