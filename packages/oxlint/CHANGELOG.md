...
- c3fdcf7: `no-catch-all-pattern` now also reports the empty object pattern `.with({}, …)` on a matcher chain: an object pattern with no keys matches every object at runtime and covers every object member of `E`, so it is the `P._` catch-all in disguise. It is never exempted — where a catch-all is genuinely needed, write `P._`.
...
- 7a8a302: Restore the `no-throw` rule, removed in 5.4.0 (#227). It is unchanged and still
